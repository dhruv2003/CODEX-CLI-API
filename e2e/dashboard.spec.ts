...
test('browser origins can be created, edited, rejected and cleared without affecting other key policy', async ({ page, request }) => {
  await page.goto(baseURL)
  await navigate(page, 'api-keys')
  await openCreate(page)
  const origins = page.locator('#create-form').getByLabel('Browser origins (optional)', {exact:true})
  await expect(origins).toHaveValue('')
  await origins.fill('http://127.0.0.1:5500, http://localhost:3000')
  const created = await submitCreate(page, 'Personal browser app')
  await expect(origins).toHaveValue('')
  await page.locator('#secret-continue').click()
  await expect(page.locator('#browser-example')).toContainText(`fetch("${baseURL}/v1/chat/completions"`)
  await expect(page.locator('#browser-example')).toContainText(created.key)
  await expect(page.locator('#browser-origins-status')).toContainText('http://127.0.0.1:5500')
  await navigate(page, 'api-keys')
  const card = page.locator(`[data-key-id="${created.id}"]`)
  await card.getByText('Edit policy for Personal browser app', {exact:true}).click()
  const editor = card.getByLabel('Browser origins (optional)', {exact:true})
  await expect(editor).toHaveValue('http://127.0.0.1:5500\nhttp://localhost:3000')
  await editor.fill('https://personal.example')
  await card.getByLabel('Requests / minute', {exact:true}).fill('25')
  await card.getByRole('button', {name:'Save policy for API key Personal browser app', exact:true}).click()
  await expect(card.locator('.meta')).toContainText(['RPM 25'])
  await card.getByText('Edit policy for Personal browser app', {exact:true}).click()
  await expect(editor).toHaveValue('https://personal.example')
  await editor.fill('https://personal.example/path')
  await card.getByRole('button', {name:'Save policy for API key Personal browser app', exact:true}).click()
  await expect(card.getByRole('alert')).toBeVisible()
  await expect(card.getByRole('alert')).toContainText(/origin/i)
  await expect(editor).toHaveValue('https://personal.example/path')
  await editor.fill('')
  await card.getByRole('button', {name:'Save policy for API key Personal browser app', exact:true}).click()
  await expect(card).toContainText('Cross-origin browser access disabled')
  await page.reload()
  await card.getByText('Edit policy for Personal browser app', {exact:true}).click()
  await expect(editor).toHaveValue('')
  const response = await request.get(`${baseURL}/admin/api-keys`, {headers:{'X-Codex-Admin':'local'}})
  expect((await response.json()).data.find((key: {id:string}) => key.id === created.id)).toMatchObject({allowedOrigins:[], requestsPerMinute:25})
  await navigate(page, 'connect')
  await expect(page.locator('#browser-example')).toContainText('YOUR_KEY')
  await expect(page.locator('#browser-example')).not.toContainText(created.key)
})

test('guided setup shares origin validation and preserves form input after rejection', async ({ page }) => {
  await page.goto(baseURL)
  await page.locator('#setup-continue').click()
  await page.locator('#wizard-next').click()
  await expectWizardStep(page, 2)
  await page.locator('#key-name').fill('Guided browser app')
  await page.locator('#key-workspace-root').fill(project)
  const origins = page.locator('#wizard-stage').getByLabel('Browser origins (optional)', {exact:true})
  await origins.fill('*')
  await page.locator('#create-form').getByRole('button', {name:'Create key', exact:true}).click()
  await expect(page.locator('#origins-error')).toBeVisible()
  await expect(page.locator('#origins-error')).toContainText(/origin/i)
  await expect(page.locator('#workspace-error')).toBeHidden()
  await expect(page.locator('#key-name')).toHaveValue('Guided browser app')
  await expect(origins).toHaveValue('*')
  await expect(page.locator('#wizard-next')).toBeDisabled()
  await origins.fill('http://localhost:3000\nhttp://127.0.0.1:5500')
  await submitCreate(page, 'Guided browser app')
  await expect(page.locator('#origins-error')).toBeHidden()
  await expect(origins).toHaveValue('')
  await expect(page.locator('#wizard-next')).toBeEnabled()
  await page.locator('#wizard-next').click()
  await expectWizardStep(page, 3)
